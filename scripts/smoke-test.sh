...
AUTH_USERNAME=$(node -e "const accounts = JSON.parse(process.argv[1]); const usernames = Object.keys(accounts); if (!usernames.length) process.exit(1); console.log(usernames[0]);" "$AUTH_ACCOUNTS_JSON")
AUTH_PASSWORD=$(node -e "const accounts = JSON.parse(process.argv[1]); const usernames = Object.keys(accounts); if (!usernames.length) process.exit(1); console.log(accounts[usernames[0]] || '');" "$AUTH_ACCOUNTS_JSON")
